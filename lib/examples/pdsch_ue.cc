...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < SRSLTE_MAX_CODEWORDS; i++) {
    data[i] = (uint8_t*)srslte_vec_u8_malloc(2000 * 8);
    if (!data[i]) {
  ...
  for (uint32_t i = 0; i < prog_args.rf_nof_rx_ant; i++) {
    sf_buffer[i] = (cf_t*)srslte_vec_cf_malloc(max_num_samples);
  }
        ...
        case DECODE_MIB:
          if (sf_idx == 0) {
            ...
            if (n < 0) {
              ...
              mac_pcap.write_dl_bch(bch_payload, SRSLTE_BCH_PAYLOAD_LEN, true, sfn * 10 + sf_idx, 0);
            }
          }
          break;
        case DECODE_PDSCH:
                  ...
                  for (uint32_t tb = 0; tb < SRSLTE_MAX_CODEWORDS; tb++) {
                    if (pdsch_cfg.grant.tb[tb].enabled) {
                      if (!acks[tb]) {
                        ...
                        if (prog_args.rnti == SRSLTE_SIRNTI) {
                          mac_pcap.write_dl_sirnti(data[tb], pdsch_cfg.grant.tb[tb].tbs, true, tti, 0);
                        } else if (prog_args.rnti == SRSLTE_PRNTI) {
                          mac_pcap.write_dl_pch(data[tb], pdsch_cfg.grant.tb[tb].tbs, true, tti, 0);
                        } else if (prog_args.rnti == SRSLTE_MRNTI) {
                          mac_pcap.write_dl_mch(data[tb], pdsch_cfg.grant.tb[tb].tbs, true, tti, 0);
                        } else if (prog_args.rnti >= SRSLTE_RARNTI_START && prog_args.rnti <= SRSLTE_RARNTI_END) {
                          mac_pcap.write_dl_ranti(data[tb], pdsch_cfg.grant.tb[tb].tbs, prog_args.rnti, true, tti, 0);
                        } else if (prog_args.rnti >= SRSLTE_CRNTI_START && prog_args.rnti <= SRSLTE_CRNTI_END) {
                          mac_pcap.write_dl_crnti(data[tb], pdsch_cfg.grant.tb[tb].tbs, prog_args.rnti, true, tti, 0);
                        }
                      }
                      if (sf_type == SRSLTE_SF_NORM) {
                        pkt_total++;
